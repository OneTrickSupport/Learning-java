...
public class Projekt extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        TableColumn<Person, String> fnamn = new TableColumn<>("Förnamn");
        fnamn.setMinWidth(150);
        fnamn.setCellValueFactory(new PropertyValueFactory<>("FNamn"));

        TableColumn<Person, String> enamn = new TableColumn<>("Efternamn");
        ...
        sokknapp.setOnAction(event -> {
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/asset?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", "root", "Mammapappa1")) {
        ...
        tabortknapp.setOnAction(event1 -> {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/asset?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", "root", "Mammapappa1")) {
                ...
                if (asset.getSelectionModel().isEmpty()) {
                    ...
                    PreparedStatement tabort = conn.prepareStatement("DELETE from person where AnställningsID = ?");
                    ...
                    tabort.setString(1, aasntällningsid);
                    ...
                    info.setContentText("Klicka på sök igen för att uppdatera listan");
                    info.showAndWait();
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/asset?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", "root", "Mammapappa1")) {
                ...
                if (rullistalaggtill.getSelectionModel().getSelectedItem().equals("Person")) {
                    ...
                    if (text1.getText().isEmpty() || text2.getText().isEmpty() || text3.getText().isEmpty() || text4.getText().isEmpty() || rulllistakontor.getSelectionModel().isEmpty()) {
                        ...
                        alert.setContentText("Fyll i textfälten och välj rummsnummer");
                        alert.showAndWait();
                    } else {
                    ...
                    if (text1.getText().isEmpty() || text2.getText().isEmpty() || text3.getText().isEmpty() || text4.getText().isEmpty() || rulllistapersoner.getSelectionModel().isEmpty()) {
                        ...
                        info.setHeaderText("Hårdvara Tillagd");
                        info.setContentText("Insättning klar");
                        info.showAndWait();
                    ...
                    if (text1.getText().isEmpty() || text2.getText().isEmpty() || text3.getText().isEmpty() || rulllistapersoner.getSelectionModel().isEmpty()) {
                        ...
                        info.setHeaderText("Mjukvara Tillagd");
                        info.setContentText("Insättning klar");
                        info.showAndWait();
